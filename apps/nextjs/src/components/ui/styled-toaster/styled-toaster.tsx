...
function handleToastError(error: unknown, custom?: string): string {
  ...
  return custom ?? "Error desconocido";
}
  ...
  <Toaster position="bottom-center" containerClassName="mb-16">
    {(t) => (
      <Transition
        appear
        show={t.visible}
        className="mb-2 flex max-w-lg items-center gap-3 rounded-md border border-border bg-background px-4 py-2 text-foreground dark:bg-background-secondary"
