...
const Root: React.FC<RootProps> = ({ children, className, isConcluded, ...props }) => {
  return (
    <div
      className={cn(
        "border-border relative flex flex-col gap-3 rounded-lg border p-6 shadow-sm",
        className,
      ...
      {isConcluded && (
        <div className="absolute left-0 top-0 z-20 flex h-full w-full items-center justify-center rounded-lg group-hover:hidden">
          <div className="flex h-full items-center justify-center">
            <BadgeCheck className="text-foreground h-12 w-12" />
          </div>
        </div>
      )}
      {children}
...
const Title: React.FC<TitleProps> = ({ children, className, ...props }) => {
  return (
    <span className={cn("max-w-[250px] text-lg font-bold", className)} {...props}>
      {children}
    </span>
  );
};
...
const AvatarContainer: React.FC<AvatarContainerProps> = ({ children, className, ...props }) => {
  return (
...
const MemberAvatar: React.FC<AvatarProps> = ({ image, fallback, className }) => {
  return (
...
const BadgeContainer: React.FC<BadgeContainerProps> = ({ children, className, ...props }) => {
  return (
    <div className={cn("flex flex-wrap items-center justify-start gap-1.5", className)} {...props}>
      {children}
    </div>
  );
};
...
const AmountBadge: React.FC<AmountBadgeProps> = ({ amount, currency, ...props }) => {
...
const DueDateBadge: React.FC<DueDateBadgeProps> = ({
  ...
  const isRecurrent = recurringFrequency !== null && duration !== null && createdAt !== null;
...
const Footer: React.FC<FooterProps> = ({ children, className, ...props }) => {
  return (
    <div
      className={cn("mt-auto flex w-full items-center justify-between gap-4", className)}
      {...props}>
      {children}
    </div>
  );
