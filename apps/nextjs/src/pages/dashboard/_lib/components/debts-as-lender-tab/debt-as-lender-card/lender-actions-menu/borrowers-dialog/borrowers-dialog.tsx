...
const BorrowersDialog: React.FC<Props> = ({ open, onOpenChange, debt }) => {
  ...
  const sendInviteForm = useForm<SendDebtInviteInput>({
    ...
    resolver: zodResolver(
      sendDebtInviteInput
        .refine((data) => data.email !== session.user?.email, {
        ...
        .refine((data) => !pendingBorrowers.some((b) => b.inviteeEmail === data.email), {
          message: "Ya existe una invitación pendiente con ese correo electrónico",
          path: ["email"],
        })
    ),
    mode: "onSubmit",
          ...
          <Form.Group>
            ...
            <div className="flex flex-col items-center gap-2 sm:flex-row">
              <Form.Input
                ...
                error={sendInviteForm.formState.errors.email?.message !== undefined}
                disabled={isArchived}
              />
            ...
            <Form.FieldError>{sendInviteForm.formState.errors.email?.message}</Form.FieldError>
          </Form.Group>
        </Form>
          ...
          {query.isFetching ? (
            ...
            <React.Fragment>
              {borrowers.map((borrower) => (
                <BorrowerRow borrower={borrower} key={borrower.user.id} currency={debt.currency} />
              ))}
